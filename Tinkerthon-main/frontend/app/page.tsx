...
export default function TeaserPage() {
  ...
  const hasSeenIntro = typeof window !== "undefined" && sessionStorage.getItem("nexus_intro_seen") === "true";
  const viewParam = searchParams.get("view");
  const skipToPortals = hasSeenIntro || viewParam === "portals";

  const [showText, setShowText] = useState(false);
  const [showGlitch, setShowGlitch] = useState(false);
  const [countdown, setCountdown] = useState(3);
  const [phase, setPhase] = useState(skipToPortals ? 2 : 0);
  const [activeView, setActiveView] = useState<ViewType>(skipToPortals ? "portals" : "hero");
  ...
  const handlePortalSelect = (sectionId: string) => {
    ...
    if (sectionId === "portals") {
      if (typeof window !== "undefined") sessionStorage.setItem("nexus_intro_seen", "true");
